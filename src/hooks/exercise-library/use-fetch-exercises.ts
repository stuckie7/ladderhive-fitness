...
export const useFetchExercises = (
  ...
  const fetchExercises = useCallback(async (): Promise<Exercise[]> => {
    ...
    try {
      ...
      if (filters.muscleGroup && filters.muscleGroup !== "all_muscle_groups") {
        const fullExercises = await fetchers.fetchExercisesFull(20, 0);
      ...
      if (filters.difficulty && filters.difficulty !== "all_difficulties") {
        filteredData = filteredData.filter(ex => ex.difficulty === filters.difficulty);
      }
  ...
  }, [filters, searchQuery, fetchers, firstMuscleGroup]);
